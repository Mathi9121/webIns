...
// Ajout dun type de formation en ajax
$("#ajouttype").click(function(e){
	e.preventDefault();
	var type = prompt("Entrez un nouveau type de formation");
	if(type){
		$.ajax({
			type: 'POST',
			...
			$("#ocim_formationsbundle_formation_type").append("<option value='"+msg+"'>"+type+"</option>");
			$("#ocim_formationsbundle_formation_type").val(msg);
		}).fail(function(msg){
			console.log(msg);
		});
	}
